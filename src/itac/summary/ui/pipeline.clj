...
(defn- draw-pipeline
  ...
  (let [{:keys [current
        ...
        ;; Farben des Startknotens, wenn aktiv
        start-style          (style :background "#859900"
                                    :foreground :black
                                    :stroke     stroke-w
                                    :cap        :round)
        ;; Farben des Endknotens, wenn aktiv
        end-style            (style :background "#dc322f"
                                    :foreground :black
                                    :stroke     stroke-w
                                    :cap        :round)

        ;; Farben des aktuellen Zustands
        current-style        (style :foreground "#268BD2"
                                    :background :white
                                    :stroke     stroke-w
                                    :cap        :round)
        ;; Farben eines bereits durchlaufenen Zustands
        active-style         (style :foreground :black
                                    :background :white
                                    :stroke     stroke-w
                                    :cap        :round)
        ;; Farben eines bisher noch nicht durchlaufenen Zustands
